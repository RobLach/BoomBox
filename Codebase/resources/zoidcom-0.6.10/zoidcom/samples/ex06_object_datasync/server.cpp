...
int main(int argc, char *argv[])
{
  ...
  while (1)
  {
    ...
    // with a small probability, change trees int variable
    // zoidcom will send the change to all client trees automatically
    if (rand() % 100 > 98)
    {
      int n = rand() % 1000;
    ...
    // with a small probability, change trees text variable
    // zoidcom will send the change to all client trees automatically
    if (rand() % 100 > 98)
    {
      if (rand() % 100 > 50)
        strcpy(tree->m_text, "Hello");
      else
        strcpy(tree->m_text, "World");

      printf("Changing text value to: %s\n", tree->m_text);
    }

    // always count up the counter variable
    tree->m_counter+=1;
