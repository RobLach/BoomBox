...
protected:
  ...
  void ZCom_cbConnectResult( ZCom_ConnID _id, eZCom_ConnectResult _result, ZCom_BitStream &_reply )
  {
    ...
    {
      ZCom_requestDownstreamLimit(_id, 30, 300);
...
int main(int argc, char *argv[])
{
  ...
  FileSender::registerClass(cli);

  // the filesender node is registered with registerNodeUnique()
  // as such, it needs to be there before the connection to the server
  FileSender *fsender = new FileSender(cli, false);
